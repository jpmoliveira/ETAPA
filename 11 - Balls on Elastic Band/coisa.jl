...
function eq_motion_dif(u, p, t)
    ...
    dθ_dot = ((adjusted_fel * sin(θ) + Fat1_y - m₁ * dry_dot) * cos(θ) - (adjusted_fel * cos(θ) + Fat1_x - m₁ * drx_dot) * sin(θ) - 2 * l_dot * θ_dot) / l
    dl_dot = ((adjusted_fel * cos(θ) + Fat1_x - m₁ * drx_dot) * cos(θ) + (adjusted_fel * sin(θ) + Fat1_y - m₁ * dry_dot) * sin(θ) + l * dθ ^ 2) / μ
...
u0 = [0., 0., 0., 1., 100., 100., 0, 0, 0, 0, 0, 0]
tspan = (0., 100.)
